...
CustomSubExplorerCommand::CustomSubExplorerCommand(const winrt::hstring& configContent, ThemeType themeType, bool enableDebug)
{
	...
	try
	{
		...
		_title_single = result.GetNamedString(L"titleSingle", L"");
...
bool CustomSubExplorerCommand::Accept(bool multipleFiles, FileType fileType, const std::wstring_view& name, const std::wstring_view& ext) {
...
IFACEMETHODIMP CustomSubExplorerCommand::GetTitle(_In_opt_ IShellItemArray* items, _Outptr_result_nullonfailure_ PWSTR* name) {
	...
	std::wstring displayTitle = _title;
	DWORD count = 0;
	if (!_title_single.empty() && items && SUCCEEDED(items->GetCount(&count)) && count == 1) {
		const auto selectedPath = PathHelper::getPath(items);
		if (!selectedPath.empty()) {
			const std::filesystem::path selectedFile(selectedPath);
			std::unordered_map<std::wstring_view, std::wstring> replacements = {
				{PARAM_PATH, selectedPath},
				{PARAM_NAME, selectedFile.filename().wstring()},
				{PARAM_PARENT, selectedFile.parent_path().wstring()},
			};
			displayTitle = PathHelper::simpleFormat(_title_single, replacements);
		}
	}
	auto title = wil::make_cotaskmem_string_nothrow(displayTitle.c_str());
