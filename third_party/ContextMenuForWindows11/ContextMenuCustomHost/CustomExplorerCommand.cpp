...
IFACEMETHODIMP CustomExplorerCommand::GetFlags(_Out_ EXPCMDFLAGS* flags) {
	...
	// The one registered entry is always the TANGERINE ZIP parent submenu.
...
IFACEMETHODIMP CustomExplorerCommand::GetTitle(_In_opt_ IShellItemArray* items, _Outptr_result_nullonfailure_ PWSTR* name) {
	*name = nullptr;
	...
	// The parent always uses the public product name; child titles are read
	// from the localized JSON definitions generated by the WPF application.
...
IFACEMETHODIMP CustomExplorerCommand::GetState(_In_opt_ IShellItemArray* selection, _In_ BOOL okToBeSlow, _Out_ EXPCMDSTATE* cmdState) {
	...
	// Discard results from a previous Explorer query before evaluating the new
	// selection; only accepted child actions make this parent visible.
	m_commands.clear();
...
IFACEMETHODIMP CustomExplorerCommand::Invoke(_In_opt_ IShellItemArray* selection, _In_opt_ IBindCtx* ctx) noexcept try {
	// The root only opens its submenu. Explorer invokes the selected child.
	return S_OK;
}CATCH_RETURN();
